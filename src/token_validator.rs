...
impl TokenValidator for BearerTokenValidator {
    fn validate(&self, request: &mut Request<Body>) -> Result<(), ProxyError> {
        ...
        let auth_header = request
            ...
            .get("authorization")
            .ok_or_else(|| {
                println!("[ERROR] [{}] Token validation error: path={}, reason=missing_authorization_header", 
                    std::thread::current().name().unwrap_or("Unknown"), self.route_path);
        ...
        let auth_str = auth_header
            .to_str()
            .map_err(|_| {
                println!("[ERROR] [{}] Token validation error: path={}, reason=invalid_authorization_header", 
                    std::thread::current().name().unwrap_or("Unknown"), self.route_path);
        ...
        if !auth_str.starts_with("Bearer ") {
            println!("[ERROR] [{}] Token validation error: path={}, reason=invalid_bearer_format", 
                std::thread::current().name().unwrap_or("Unknown"), self.route_path);
            return Err(ProxyError::Unauthorized("Invalid Bearer token format".to_string()));
        }
        ...
        if !self.verified_tokens.contains(&token) {
            ...
            println!("[WARN] [{}] Token validation failed: path={}, token={}, reason=not_in_whitelist", 
                std::thread::current().name().unwrap_or("Unknown"), self.route_path, masked_token);
        ...
        let new_header_value = HeaderValue::from_str(&new_auth_header)
            .map_err(|_| ProxyError::InternalError("Failed to create new auth header".to_string()))?;
        
        request.headers_mut().insert("authorization", new_header_value);

        // 更新计数器
        let count = self.success_count.fetch_add(1, Ordering::SeqCst) + 1;

        let masked_new = if new_token.len() > 6 {
            format!("{}***{}", &new_token[..3], &new_token[new_token.len()-3..])
        } else {
            "***".to_string()
        };

        println!("[INFO] [{}] Token validation success: path={}, token={}⟶{}, count={}", 
            std::thread::current().name().unwrap_or("Unknown"), 
            self.route_path, masked_original, masked_new, count);
